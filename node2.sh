...
then
  echo $PRIVATE_KEY_2 >> /tmp/key2
  wallet2=$(${PROJECT_DIR}/build/bin/$Bin_NAME account import --password .pwd --datadir ./nodes/2 /tmp/key2 | awk -v FS="({|})" '{print $2}')
...
${PROJECT_DIR}/build/bin/$Bin_NAME --bootnodes "$BOOTNODE" --syncmode "full" --datadir ./nodes/2 --networkid $networkid --port 30304 --rpc --rpccorsdomain "*" --ws --wsaddr="0.0.0.0" --wsorigins "*" --wsport 8556 --rpcaddr 0.0.0.0 --rpcport 8546 --rpcvhosts "*" --unlock "${wallet2}" --password ./.pwd --mine --gasprice "${GASPRICE}" --ipcdisable --targetgaslimit "420000000" --verbosity ${VERBOSITY} --rpcapi admin,db,eth,debug,miner,net,shh,txpool,personal,web3,XDPoS # --ethstats "XinFin-MasterNode-02:xinfin_network_stats@stats.testnet.xinfin.network:3000" 
# child_proc="$child_proc $!"
